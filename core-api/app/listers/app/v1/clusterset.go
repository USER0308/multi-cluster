...
type ClusterSetLister interface {
	...
	List(selector labels.Selector) (ret []*v1.ClusterSet, err error)
...
// List lists all ClusterSets in the indexer.
func (s *clusterSetLister) List(selector labels.Selector) (ret []*v1.ClusterSet, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.ClusterSet))
	})
	return ret, err
}
...
type ClusterSetNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.ClusterSet, error)
...
// List lists all ClusterSets in the indexer for a given namespace.
func (s clusterSetNamespaceLister) List(selector labels.Selector) (ret []*v1.ClusterSet, err error) {
...
// Get retrieves the ClusterSet from the indexer for a given namespace and name.
func (s clusterSetNamespaceLister) Get(name string) (*v1.ClusterSet, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("clusterset"), name)
	}
	return obj.(*v1.ClusterSet), nil
}
